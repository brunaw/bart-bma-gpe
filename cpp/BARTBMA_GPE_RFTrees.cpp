...
//  M(grow_node-1,6)=0;
  ...
  List::create(rname,
               CharacterVector::create("left daughter","right daughter","split var","split point","status","mean"));
  // and assign it
...
double find_prev_nonterm_gpv(arma::uvec find_nonterm,NumericVector prev){
               ...
               NumericVector terminal_nodes,NumericVector grow_obs,double d,NumericVector get_min,arma::mat& data_curr_node)
{
  ...
  if(prior_tree_table_temp.nrow()==grow_node){
    ...
    if(prev.size()!=0){
      ...
      prev_nonterm=find_prev_nonterm_gpv(find_nonterm,prev);
    }
...
NumericMatrix start_tree(double start_mean,double start_sd){
  ...
  NumericMatrix treemat(1,6);
  double rand=R::rnorm(start_mean,start_sd);
  NumericVector testrow = NumericVector::create(0,0,0,0,-1,rand);
  for(int k=0;k<1;k++){
    for(int j=0;j<6;j++){
...
NumericVector get_testdata_term_obs(NumericMatrix test_data,NumericMatrix tree_data,NumericVector term_node_means) {
  ...
  // std::cout<<"index for tree list is "<<j<<"\n";
  
  NumericMatrix curr_tree=sum_treetable[j];
  // std::cout<<"see this? means curr tree is a matrix! "<<"\n";
  NumericMatrix curr_obs_nodes=sum_obs_to_nodes[j];
  //  std::cout<<"curr obs nodes is a matrix! "<<"\n";
  NumericVector tree_term_nodes=find_term_nodes(curr_tree);
  int b_j=tree_term_nodes.size();
  //will make J as we go in BART-BMA no need to create it again here....
  //  std::cout<<"about to go into J fun "<<"\n";
  arma::mat Jmat=J(curr_tree,curr_obs_nodes,tree_term_nodes);
  //  std::cout<<"upsilon is "<<upsilon<<" W ncol is "<<W.n_cols<<"\n";
  // std::cout<<"n is "<<n<<" Jmat nrow "<<Jmat.n_rows<<"\n";
  ...
  double rand_eff_likelihood_function(NumericVector y_temp,NumericMatrix treetable_temp,NumericMatrix obs_to_nodes_temp,double a,double mu,double nu,double lambda,arma::mat psi_inv){
  //find terminal nodes of the tree
  NumericVector tree_term_nodes=find_term_nodes(treetable_temp);
  double n=obs_to_nodes_temp.nrow();
  double b=tree_term_nodes.size();
  arma::vec yvec=Rcpp::as<arma::vec>(y_temp);
  arma::mat y(n,1);
  y.col(0)=yvec;
  //make J matrix
  arma::mat Jmat=J(treetable_temp,obs_to_nodes_temp,tree_term_nodes);
  //get exponent
  double expon=(n+nu+b)/2;
  //get y^Tpsi^{-1}y
  //arma::mat psi_inv=psi.i();
  arma::mat ypsiy=y.t()*psi_inv*y;
  
  //get t(y)inv(psi)J
  arma::mat ypsiJ=y.t()*psi_inv*Jmat;
  //get t(J)inv(psi)J  
  arma::mat jpsij=Jmat.t()*psi_inv*Jmat;
  //get jpsij +aI
  arma::mat aI(b,b);
  aI=a*aI.eye();
  arma::mat sec_term=jpsij+aI;
  arma::mat sec_term_inv=sec_term.i();
  //get t(J)inv(psi)y
  arma::mat third_term=Jmat.t()*psi_inv*y;
  //get m^TV^{-1}m
  arma::mat mvm= ypsiJ*sec_term_inv*third_term;
  arma::mat rel=-expon*log(nu*lambda - mvm +ypsiy);
  ...
  double rand_eff_RF_likelihood_function(NumericVector y_temp,NumericMatrix treetable_temp,arma::mat Jmat,double a,double mu,double nu,double lambda,arma::mat psi_inv){
  //find terminal nodes of the tree
  //NumericVector tree_term_nodes=find_term_nodes(treetable_temp);
  double n=Jmat.n_rows;
  double b=Jmat.n_cols;
  arma::vec yvec=Rcpp::as<arma::vec>(y_temp);
  arma::mat y(n,1);
  y.col(0)=yvec;
  //make J matrix
  ...
  // [[Rcpp::depends(RcppArmadillo)]]
  // [[Rcpp::export]]
  double sumtree_likelihood_function(NumericVector y_temp,List sum_treetable ,List sum_obs_to_nodes,int n,double a,double nu,double lambda,arma::mat psi_inv){
  // std::cout<<"inside sum tree likelihood "<<"\n";
  ...
  //get y^Tpsi^{-1}y
  //arma::mat psi_inv=psi.i();
  arma::mat yty=y.t()*psi_inv*y;
  
  //get t(y)inv(psi)J
  arma::mat ytW=y.t()*psi_inv*Wmat;
  //get t(J)inv(psi)J  
  arma::mat WtW=Wmat.t()*psi_inv*Wmat;
  //get jpsij +aI
  arma::mat aI(b,b);
  aI=a*aI.eye();
  ...
  arma::mat sec_term_inv=sec_term.i();
  //get t(J)inv(psi)y
  arma::mat third_term=Wmat.t()*psi_inv*y;
  //get m^TV^{-1}m
  ...
  List get_best_split(NumericVector resids,arma::mat& data,NumericMatrix treetable,NumericMatrix tree_mat,double a,double mu,double nu,double lambda,int c,double lowest_BIC,int parent,NumericMatrix cp_mat,double alpha,double beta,int maxOWsize,int first_round,arma::mat psi_inv){
  //this function will search through all predictive split points and return those within Occam's Window.
  std::cout<<"in best split"<<"\n"; 
  //Progress ps(0,false);
  int split_var;
  ...
  double lik;
  std::cout<<"going to loop through term nodes"<<"\n";
  for(int l=0;l<terminal_nodes.size();l++){
     //if (Progress::check_abort() ){
      //  throw std::range_error("User termination");
    //}
    ...
    if(data_curr_node.n_rows<=2){
      std::cout<<"l is "<<l<<" terminal nodes size is "<<terminal_nodes.size()<<"\n";
      //  throw std::range_error("not enough obs in node to grow any further");
      continue;
    }
    for(int k=0;k<w;k++){
    //  std::cout<<"k is "<<k<<" of "<<w<<" in termnode "<<l<<" of "<<terminal_nodes.size() <<"\n";
     //if (Progress::check_abort() ){
      //  throw std::range_error("User termination in k loop");
    // }
      ...
      std::cout<<"going to grow proposal tree"<<"\n";
      ...
      std::cout<<"did it proposal grown"<<"\n";
      ...
      if(first_round==1){
        lik=rand_eff_likelihood_function(resids,proposal_tree[0],proposal_tree[1],a,mu,nu,lambda,psi_inv);
        
      }else{
        //have a sum of trees
        throw std::range_error("Error: likelihood should be calculated for first round trees only in get_best_split function");
        //lik=likelihood_function(resids,proposal_tree[0],proposal_tree[1],a,mu,nu,lambda);  
        // std::cout<<"in best split lik is "<<lik<<"\n";
      }
    ...
    if(testlik.size()>0){
      ...
      for(int l=0;l<terminal_nodes.size();l++){
        ...
        for(int k=0;k<w;k++){
          ...
          //   std::cout<<"about to grow proposal tree"<<"\n";
          proposal_tree=grow_tree(data,resids,treemat_c,terminal_nodes[l],treetable_c,split_var,split_point,terminal_nodes,wrap(grow_obs),d,get_min,data_curr_node);
          NumericMatrix test =proposal_tree[0];
          NumericMatrix test1 =proposal_tree[1];
          ...
          if(first_round==1){
            throw std::range_error("Error: Likelihood should be called from get_best_split for first round");
            //lik=rand_eff_likelihood_function(resids,proposal_tree[0],proposal_tree[1],a,mu,nu,lambda,psi);
          }else{
            ...
            if(is<List>(s)){
              ...
              lik=sumtree_likelihood_function(y_scaled,sum_trees2,sum_trees_mat2,y_scaled.size(),a,nu,lambda,psi_inv);  
              ...
              // return(st);
              lik=sumtree_likelihood_function(y_scaled,st,st_mat,y_scaled.size(),a,nu,lambda,psi_inv); 
            ...
            //  std::cout<<"about to get likelihood function"<<"\n";
      ...
      std::cout<<"leaving best splits, tree grown"<<"\n";
      tree_list=resize(tree_list,count);
      tree_mat_list=resize(tree_mat_list,count);
      tree_lik.resize(count);
      tree_parent.resize(count);
          ...
          if(ld_size.size()>2 && rd_size.size()>2)
{
          ...
          // [[Rcpp::depends(RcppArmadillo)]]
          // [[Rcpp::export]]
          List make_gridpoint_cpmat(NumericMatrix data,NumericVector resp,int gridsize,int num_cp,int max_cp){
            //will create a list of the minimum of top num_cp% split points or max_cp
          int err1=0;
          ...
          //std::cout<<"changepoint nrows "<<change_points.n_rows<<"\n";
          if(row_count+1!=(int) change_points.n_rows){
          //   std::cout<<"row count +1 "<<row_count+1<<"\n";
          ...
          // [[Rcpp::depends(RcppArmadillo)]]
          // [[Rcpp::export]]
          List make_pelt_cpmat(NumericMatrix data,NumericVector resp,double pen,int num_cp,int max_cp){
          int err1=0;
          int n=data.nrow();
          ...
          if(xLeft.size()>2 && xRight.size()>2)
{
          ...
          int first_round,IntegerVector parent,List cp_mat_list,IntegerVector err_list,NumericMatrix test_data,double alpha,double beta,bool is_test_data,double pen,int num_cp,bool split_rule_node,bool gridpoint,int maxOWsize,arma::mat psi_inv,int max_cp
          ){
          std::cout<<"inside best trees BIC is "<<lowest_BIC<<"\n";
          List eval_model;
          ...
          std::cout<<"inside tree table.size loop lowest BIC is "<<lowest_BIC<<"\n";
          best_subset=get_best_split(resids(_,0),D1,tree_table[i],tree_mat[i],a,mu,nu,lambda,log(c),lowest_BIC,parent[0],cp_mat_list[0],alpha,beta,maxOWsize,first_round,psi_inv); 
          std::cout<<"left best split "<<"\n";
          if(best_subset.size()==1){
          std::cout<<"best subset size was 1"<<"\n";
          continue;
          }
          std::cout<<"check we got past if statement"<<"\n";
          List temp_trees=best_subset[4];
          List temp_mat=best_subset[6];
          lik_list=best_subset[5];
          IntegerVector temp_parent=best_subset[7];
          std::cout<<"check before temp parent size"<<"\n";
          if(temp_parent.size()!= temp_trees.size()){
          throw std::range_error("there should be a parent for each tree!!!");
          }
          std::cout<<"and after it"<<"\n";
          if(lik_list.size()==0){
          ...
          std::cout<<"after finding best splits lowest BIC is "<<min(lik_list)<<"\n";
          ...
          //  std::cout<<"tree size before evaluate model "<<overall_lik.size()<<"\n";
          ...
          List cp_mat_list1;
          if(gridpoint==0){
          cp_mat_list1=make_pelt_cpmat(wrap(D1),curr_resids(_,f),pen,num_cp,max_cp);
          }else{
          cp_mat_list1=make_gridpoint_cpmat(wrap(D1),curr_resids(_,f),pen,num_cp,max_cp);
          }
          ...
          List get_best_trees_sum(
              arma::mat& D1,NumericMatrix resids,double a,double mu,double nu,
              double lambda,int c,double sigma_mu,List tree_table,List tree_mat,
              double lowest_BIC,
              int first_round,IntegerVector parent,List cp_mat_list,
              IntegerVector err_list,NumericMatrix test_data,double alpha,double beta,
              bool is_test_data,double pen,int num_cp,bool split_rule_node,bool gridpoint,
              int maxOWsize,List prev_sum_trees,List prev_sum_trees_mat,
              NumericVector y_scaled,arma::mat psi_inv,int max_cp
          ){
          List eval_model;
          NumericVector lik_list;
          List best_subset;
          ...
          for(int j=0;j<5;j++){
          std::cout<<"inner round is "<<j<<"in best trees sum total number of trees to loop through "<<tree_table.size()<<"\n";
          int lsize=1000;
          List table_subset_curr_round(lsize);
          std::vector<double> lik_subset_curr_round(lsize);
          List mat_subset_curr_round(lsize);
          ...
          parent=-1;
          NumericMatrix temp_list=cp_mat_list[0];
          throw std::range_error("get_best_splits should be called from get_best_trees");
          //	best_subset=get_best_split(resids(_,0),D1,tree_table[i],tree_mat[i],a,mu,nu,lambda,log(c),lowest_BIC,parent[0],cp_mat_list[0],alpha,beta,maxOWsize,first_round,psi); 
          }else{
          if(err_list[i]==0){
          //   std::cout<<"is tree table i a matrix?"<<"\n";
          NumericMatrix test_tree=tree_table[i];
          //  std::cout<<"is test treemat a matrix?"<<"\n";
          NumericMatrix test_treemat=tree_mat[i];
          //  std::cout<<"is testcpmat a matrix?"<<"\n";
          NumericMatrix test_cpmat= cp_mat_list[parent[i]];
          //  std::cout<<"about to get best split sum"<<"\n";
          //need to append current tree_table[i] to its parent sum_of_trees  
          std::cout<<"about to enter best splits "<<"\n";
          best_subset=get_best_split_sum(resids(_,parent[i]),D1,tree_table[i],tree_mat[i],a,mu,nu,lambda,log(c),lowest_BIC,parent[i],cp_mat_list[parent[i]],alpha,beta,maxOWsize,first_round,prev_sum_trees,prev_sum_trees_mat,y_scaled,parent,i,psi_inv);    
          std::cout<<"about to return best subset "<<best_subset.size()<<"\n";
          // return(best_subset);
          ...
          std::cout<<"temp_trees.size() "<<temp_trees.size()<<"\n";
          for(int k=0;k<temp_trees.size();k++){
          table_subset_curr_round[count]=temp_trees[k];
          lik_subset_curr_round[count]=lik_list[k];
          mat_subset_curr_round[count]=temp_mat[k];
          ...
          ret[6]=lowest_BIC;
          ret[7]=wrap(M2);
          std::cout<<"leaving best trees sum"<<"\n";
          return(ret);
          }
          //######################################################################################################################//
          // [[Rcpp::export]]
          ...
          // [[Rcpp::depends(RcppArmadillo)]]
          // [[Rcpp::export]]
          List BART_BMA_sumLikelihood_GPV(NumericMatrix data,NumericVector y,double start_mean,double start_sd,double a,double mu,double nu,double lambda,int c,
          double sigma_mu,double pen,int num_cp,NumericMatrix test_data,int num_rounds,double alpha,double beta,bool split_rule_node,bool gridpoint,int maxOWsize,arma::mat psi_inv,int max_cp){
          bool is_test_data=0;
          if(test_data.nrow()>0){
          is_test_data=1;
          }
          ...
          double lik=rand_eff_likelihood_function(y_scaled,treetable,treemat,a,mu,nu,lambda,psi_inv);
          ...
          for(int j=0;j<num_rounds;j++){
            ...
            for(int f=0;f<resids.ncol();f++){
              if(gridpoint==0){
                cp_mat_list=make_pelt_cpmat(data,resids(_,f),pen,num_cp,max_cp);
              }else{
               cp_mat_list=make_gridpoint_cpmat(data,resids(_,f),pen,num_cp,max_cp);
              }
            ...
            */
            ...
            if(j==0){
             // CART_BMA=get_best_trees(get_rf_trees,resids,D1,)
            CART_BMA=get_best_trees(D1, resids, a,mu,nu,lambda,c,sigma_mu,tree_table,tree_mat,
                                    lowest_BIC,first_round,parent,resids_cp_mat,as<IntegerVector>(wrap(err_list)),
                                    test_data,alpha,beta,is_test_data,pen,num_cp,split_rule_node,gridpoint,maxOWsize,psi_inv,max_cp);
            return(CART_BMA);
            }else{
            //if j >0 then sum of trees become a list so need to read in list and get likelihood for each split point and terminal node
            std::cout<<"going to get best tree sum "<<"\n";
            CART_BMA=get_best_trees_sum(
              D1, resids, a,mu,nu,lambda,c,sigma_mu,tree_table,tree_mat,
              lowest_BIC,first_round,parent,resids_cp_mat,
              as<IntegerVector>(wrap(err_list)),test_data,alpha,beta,
              is_test_data,pen,num_cp,split_rule_node,gridpoint,maxOWsize,prev_sum_trees,
              prev_sum_trees_mat,y_scaled,psi_inv,max_cp);
            std::cout<<"got back from best tree sum "<<"\n";
            }
            ...
            //		}
            //   if(j==1){
            std::cout<<"return overall "<<"\n";
            //    return(CART_BMA);
            //  }
            curr_round_lik=CART_BMA[0];
            std::cout<<"curr_round_lik is "<<curr_round_lik.size()<<" "<<curr_round_lik[0]<<"cart_bma "<<CART_BMA.size()<<"\n";
            ...
            curr_BIC=CART_BMA[6];
            std::cout<<"curr_round_BIC is "<<curr_BIC.size()<<" "<<curr_BIC[0]<<"\n";
            ...
            int count=0; 
            std::cout<<"about to enter k loop "<<"\n";
           ...
           // std::cout<<"out of k loop"<<"\n";
            if(curr_round_lik.size()==0){
            throw std::range_error("No trees chosen in last round");
            }
           // std::cout<<"going to go through curr round lik again "<<curr_round_lik.size()<<"\n";
            for(int k=0;k<curr_round_lik.size();k++){
            int size_mat=300;
            ...
            std::cout<<"went through curr round like "<<overall_sum_BIC.size()<<"\n";
            ...
            std::cout<<"checked for previous round children "<<overall_sum_BIC.size()<<"\n";
            ...
            std::cout<<"getting overall overall sum trees"<<"\n";
            ...
            oo_count ++;
            std::cout<<"check oocount size "<< oo_count<<" "<<overall_sum_BIC.size()<<"\n";
          ...
          std::cout<<"looping through endBIC "<<end_BIC.size()<<"\n";
          
          for(int k=0;k<end_BIC.size();k++){
          //std::cout<<"k is "<<k<<" "<<end_BIC.size()<<" "<<prev_round_preds.ncol()<<"\n";
         ...
         // std::cout<<"BICi length "<<BICi.size()<<overall_overall_sum_test_preds.n_cols<<"\n";
