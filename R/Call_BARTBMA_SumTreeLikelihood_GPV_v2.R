BART_BMA_GPV=function(x.train,y.train,a=3,nu=3,sigquant=0.9,c=1000,
                      pen=12,num_cp=20,x.test=matrix(0.0,0,0),num_rounds=5,alpha=0.95,beta=1,split_rule_node=0,gridpoint=0,maxOWsize=500,psi,max_cp=50
){
  ...
  if(nrow(psi)!=nrow(x.train) ||ncol(psi)!=nrow(x.train)){
    stop("Psi must be an nxn matrix of genetic relatedness");
  }
  #read in the inverse of psi to BARTBMA call to make run time more efficient
  psi_inv<-solve(psi)
  start=proc.time()
  print("Got here")
  test_BMA34=BART_BMA_sumLikelihood_GPV(x.train,y.train,start_mean,start_sd,a,mu,nu,lambda,c,sigma_mu,
                                        pen,num_cp,x.test,num_rounds,alpha,beta,split_rule_node,gridpoint,maxOWsize,psi_inv,max_cp)
  end=proc.time()
